...
function guardarGrupos(data) {
  escribirJSONAtomico(RUTA_DB_GRUPOS, data)
}
...
function registrarAvisoAntilink(numero) {
  const data = leerTodo()
  const actual = data[numero] || {}
  const avisos = (actual.avisosAntilink || 0) + 1
  data[numero] = { ...actual, avisosAntilink: avisos, actualizado: Date.now() }
  guardarTodo(data)
  return avisos
}

function reiniciarAvisosAntilink(numero) {
  const data = leerTodo()
  const actual = data[numero] || {}
  data[numero] = { ...actual, avisosAntilink: 0, actualizado: Date.now() }
  guardarTodo(data)
}
...
module.exports = {
  ...
  agregarMonedas,
  agregarMonedasConBoost,
  reclamarDaily,
  comprarItem,
  equiparItem,
  tieneProteccionAntiflood,
  registrarAvisoAntilink,
  reiniciarAvisosAntilink,
}