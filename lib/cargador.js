 ...
 *
 * Si un hook devuelve `true` (ej: modoadmi, botgrupo bloqueando a un
 * usuario normal), se detiene la cadena de hooks Y se le indica al
 * llamador (main.js/subbot.js) que NO debe despachar el comando.
 * Cualquier hook que no devuelva nada (undefined) no afecta este flujo.
 */
async function ejecutarHooks(lista, ...args) {
  for (const hook of lista) {
    try {
      const resultado = await hook.fn(...args);
