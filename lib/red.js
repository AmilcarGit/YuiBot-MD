...
async function actualizarGruposPrincipal(sock) {
  try {
    ...
    escribirJSON(RUTA_GRUPOS_PRINCIPAL, {
      grupos: ids,
    ...
    if (ahora - ultimoErrorLogueado > 300000) {
      ultimoErrorLogueado = ahora

      console.error(
        '[NETWORK] No se pudo refrescar la lista de grupos del principal (se sigue usando la última lista conocida):',
        error.message
      )
    }
  }
...
function principalPresenteEnGrupo(groupId) {
  ...
  if ((Date.now() - info.actualizado) > GRUPOS_TTL_MS) {
    return false
  }
...
function intentarAdquirirLock(groupId, numero) {
  ...
  try {
    ...
    console.error(
      '[NETWORK] Error inesperado adquiriendo LOCK:',
      error.message
    )
...
function puedeResponderSubbot(numero, groupId) {
  if (principalPresenteEnGrupo(groupId)) {
    logSiCambia(
      groupId,
      'principal',
      `Principal detectado en grupo ${groupId}: subbot ${numero} en espera.`
    )
  ...
  if (lock && heartbeatFresco(lock.owner)) {
    if (lock.owner === numero) {
      logSiCambia(
        groupId,
        `lider:${numero}`,
        `Subbot ${numero} mantiene el LOCK del grupo ${groupId}.`
      )
    ...
    logSiCambia(
      groupId,
      `esperando:${lock.owner}`,
      `Subbot ${numero} ignorado: LOCK pertenece a ${lock.owner}.`
    )
  ...
  if (lock) {
    try {
      fs.unlinkSync(rutaLock(groupId))
    } catch {}

    console.log(
      `[NETWORK] LOCK del grupo ${groupId} expiró (${lock.owner} sin heartbeat). Liberado.`
    )
  }

  console.log(
    `[NETWORK] Subbot ${numero} intentando adquirir LOCK del grupo ${groupId}...`
  )
  ...
  if (gane) {
    logSiCambia(
      groupId,
      `lider:${numero}`,
      `LOCK adquirido por ${numero} en grupo ${groupId}.`
    )
  ...
  logSiCambia(
    groupId,
    'perdido-race',
    `Subbot ${numero} no obtuvo el LOCK (otro subbot lo ganó primero).`
  )
    ...
    for (const archivo of archivos) {
      try {
        const lock = JSON.parse(
          fs.readFileSync(
            path.join(CARPETA_LOCKS, archivo),
            'utf-8'
          )
        )
