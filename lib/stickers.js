...
async function crearStickerWebp(buffer, { animado = false, config, extensionEntrada } = {}) {
  const idTmp = `${Date.now()}-${Math.random().toString(36).slice(2)}`
  const ext = extensionEntrada || (animado ? 'mp4' : 'jpg')
  const entrada = path.join(os.tmpdir(), `stk-in-${idTmp}.${ext}`)
