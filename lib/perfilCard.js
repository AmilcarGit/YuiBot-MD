...
async function generarTarjetaPerfil({ numero, nombre, genero, edad, avatar, background, botName }) {
  const ancho = 1024
  const alto = 450
  ...
  textoConSombra(ctx, nombre.toUpperCase(), 40, 320)
  ...
  const lineaExtra = genero ? `${genero} · ${edad} años` : `+${numero}`
  textoConSombra(ctx, lineaExtra, 40, 375)

  ctx.textAlign = 'right'
  ctx.font = 'bold 26px sans-serif'
  ctx.fillStyle = '#e0b0b0'
  textoConSombra(ctx, '⛧ PERFIL ⛧', ancho - 40, 95)

  ctx.font = 'bold 40px sans-serif'
  ctx.fillStyle = '#ffffff'
  textoConSombra(ctx, `+${numero}`, ancho - 40, 145)
...
module.exports = { generarTarjetaPerfil }